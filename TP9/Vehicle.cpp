...
float Vehicle::moveTo(double x, double y) { // todo comment accéder à octaneRate ?
    float distance = sqrt(pow((position.getX()-x),2) + pow((position.getY()-y),2));
//    if(octaneRate = 98) {
//        float neededFuel = ((distance*consumption/100)*0.97);
//    }
//    else {
...
Vehicle::Vehicle(Vehicle const &v) : position(v.position) {
    cout << "Vehicle::Vehicle(Vehicle const &v) : " << this << endl;
    this->id = v.id;
    this->fuel = v.fuel;
    this->consumption = v.consumption;
}
